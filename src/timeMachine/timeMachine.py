...
class TimeMachine(TableIndex):
    ...
    def statistics(self, verbose: int = 0) -> str:
        ...
        """
        s = self.__str__()
        versions = defaultdict(int)  # Dict(1)				# {len(windows): count, ...}
        attrs = defaultdict(int)  # Dict(1)					# {attribute: count of value changes}
        for windows in self.d.values():
        ...
        values = defaultdict(lambda: defaultdict(int))  # Dict(2, 0)  			# {attr: {value: count}}
